...
void Misc_Tick() {
    ...
    // --- No Hostile Peds ---
    if (noHostilePeds) {
        if (!relationshipsAltered) {
            Ped playerPed = PLAYER::PLAYER_PED_ID();
            Hash playerGroup = PED::GET_PED_RELATIONSHIP_GROUP_HASH(playerPed);
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(2, playerGroup, GAMEPLAY::GET_HASH_KEY("CIVMALE"));
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(2, playerGroup, GAMEPLAY::GET_HASH_KEY("CIVFEMALE"));
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(2, playerGroup, GAMEPLAY::GET_HASH_KEY("COP"));
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(2, playerGroup, GAMEPLAY::GET_HASH_KEY("ARMY"));
            relationshipsAltered = true;
        }
        ...
        if (relationshipsAltered) {
            Ped playerPed = PLAYER::PLAYER_PED_ID();
            Hash playerGroup = PED::GET_PED_RELATIONSHIP_GROUP_HASH(playerPed);
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(0, playerGroup, GAMEPLAY::GET_HASH_KEY("CIVMALE"));
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(0, playerGroup, GAMEPLAY::GET_HASH_KEY("CIVFEMALE"));
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(255, playerGroup, GAMEPLAY::GET_HASH_KEY("COP"));
            PED::SET_RELATIONSHIP_BETWEEN_GROUPS(255, playerGroup, GAMEPLAY::GET_HASH_KEY("ARMY"));
            relationshipsAltered = false;
        }
    }
...
void Misc_DrawMenu(int& menuIndex, float x, float y, float w, float h) {
    // NEW: Updated menu options for the follower system
    static const char* miscOpts[] = {
        "Slow Motion", "Refill Health/Armor", "Give $1,000,000", "Wanted Up", "Wanted Down",
        "Bullet Explosion Type", "Hash Gun", "Populate Now", "Peds Follow You", "Follower Count", "All Peds Follow You", "No Hostile Peds", "No Clip",
        "Save Current Coords"
    };
    bool* miscToggles[] = {
    ...
    auto drawOption = [&](int idx, const char* name, bool* toggle, float cy, bool active = false) {
        ...
        if (toggle) {
            if (toggle != &savePlayerCoords && toggle != &refillHPArmor && toggle != &moneyCheat && toggle != &wantedUp && toggle != &wantedDown) {
                char buf[64];
    ...
    for (int i = 0; i < miscNum; ++i) {
        ...
        if (i == 5) { // Bullet Explosion Type
            GRAPHICS::DRAW_RECT(x + w * 0.5f, cy + h * 0.5f, w, h - 0.004f, active ? 120 : 100, active ? 180 : 100, active ? 230 : 180, active ? 255 : 170);
            UI::SET_TEXT_FONT(0); UI::SET_TEXT_SCALE(0.0f, 0.34f); UI::SET_TEXT_COLOUR(active ? 20 : 20, active ? 40 : 20, active ? 130 : 110, 255);
            UI::_SET_TEXT_ENTRY("STRING"); UI::_ADD_TEXT_COMPONENT_STRING((char*)"Bullet Explosion Type"); UI::_DRAW_TEXT(x + 0.017f, cy + 0.007f);
            char buf[64]; sprintf(buf, "< %s >", bulletTypeNames[bulletExplosionType]);
            UI::SET_TEXT_FONT(0); UI::SET_TEXT_SCALE(0.0f, 0.33f); UI::SET_TEXT_COLOUR(active ? 0 : 0, active ? 255 : 200, 200, 255); UI::SET_TEXT_CENTRE(1);
            UI::_SET_TEXT_ENTRY("STRING"); UI::_ADD_TEXT_COMPONENT_STRING(buf); UI::_DRAW_TEXT(x + w - 0.065f, cy + 0.008f);
        }
        else if (i == 9) { // Follower Count
            GRAPHICS::DRAW_RECT(x + w * 0.5f, cy + h * 0.5f, w, h - 0.004f, active ? 120 : 100, active ? 180 : 100, active ? 230 : 180, active ? 255 : 170);
            UI::SET_TEXT_FONT(0); UI::SET_TEXT_SCALE(0.0f, 0.34f); UI::SET_TEXT_COLOUR(active ? 20 : 20, active ? 40 : 20, active ? 130 : 110, 255);
            UI::_SET_TEXT_ENTRY("STRING"); UI::_ADD_TEXT_COMPONENT_STRING((char*)"Follower Count"); UI::_DRAW_TEXT(x + 0.017f, cy + 0.007f);
            char buf[64]; sprintf(buf, "< %d >", followerCount);
            // Gray out the text if followAllPeds is enabled
            UI::SET_TEXT_FONT(0); UI::SET_TEXT_SCALE(0.0f, 0.33f); UI::SET_TEXT_COLOUR(followAllPeds ? 150 : (active ? 0 : 0), followAllPeds ? 150 : (active ? 255 : 200), followAllPeds ? 150 : 200, 255); UI::SET_TEXT_CENTRE(1);
            UI::_SET_TEXT_ENTRY("STRING"); UI::_ADD_TEXT_COMPONENT_STRING(buf); UI::_DRAW_TEXT(x + w - 0.065f, cy + 0.008f);
        }
        else {
            drawOption(i, miscOpts[i], miscToggles[i], cy, active);
        }
    }

    if (IsKeyJustUp(VK_NUMPAD8) || PadPressed(DPAD_UP))
        menuIndex = (menuIndex - 1 + miscNum) % miscNum;
    if (IsKeyJustUp(VK_NUMPAD2) || PadPressed(DPAD_DOWN))
        menuIndex = (menuIndex + 1) % miscNum;

    // Handle left/right for sliders
    if (menuIndex == 5 && dir != 0) { // Bullet Explosion
        bulletExplosionType = (bulletExplosionType + dir + bulletTypeCount) % bulletTypeCount;
    }
    if (menuIndex == 9 && dir != 0 && !followAllPeds) { // Follower Count
        followerCount += dir;
        if (followerCount < 1) followerCount = 1;
        if (followerCount > 20) followerCount = 20;
    }

    // Handle activation
    if (IsKeyJustUp(VK_NUMPAD5) || PadPressed(BTN_A)) {
        if (menuIndex != 5 && menuIndex != 9 && miscToggles[menuIndex] != nullptr) {
            ...
            *pToggle = !*pToggle;
            ...
            if (pToggle == &noClipMode && !*pToggle) {
                ...
                if (ENTITY::DOES_ENTITY_EXIST(playerPed)) {
                    ...
                    float ground_z;
                    if (GAMEPLAY::GET_GROUND_Z_FOR_3D_COORD(pos.x, pos.y, pos.z + 1.0f, &ground_z, false)) {
                        ENTITY::SET_ENTITY_COORDS_NO_OFFSET(playerPed, pos.x, pos.y, ground_z + 1.0f, false, false, true);
                    }
                }
            }
        }
